...
module.exports = async (data) => {
  data.forEach(element => {
    ...
    dbCollectionCurrent.findOne({ id: element.id }, async (err, doc) => {
      if (err) {
      ...
      logger('info', ['create-or-update-db', 'insert', `inserted update for ${history.id} to history`])

      if (doc) {
        // Update existing
        dbCollectionCurrent.update({ _id: mongojs.ObjectId(doc._id) }, roomStatus)
        logger('info', ['create-or-update-db', 'update', `updated ${roomStatus.id}`])
      } else {
        // Create new object
        ...
        logger('info', ['create-or-update-db', 'insert', `inserted new object: ${roomStatus.id} / ${roomStatus.displayName}`])
      }
    })
  })
}
